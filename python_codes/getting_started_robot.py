...
import pandas as pd
import matplotlib
matplotlib.use("Agg") # displayがないエラーを防ぐ
...
asset_path = "/home/shinsakuo/workspace/Data/koch.usd"
...
dof = int(np.array(arm.get_joints_state().positions).size)
...
for i in range(4):
    ...
    if i == 1 or i == 3:
        print("moving")
        # move the arm
        arm.set_joint_positions([[-1.5, 0.0, 0.0, -1.5, 0.0, 1.5]])
        # move the car
    ...
    for j in range(100):
        ...
        pos = np.array(arm.get_joints_state().positions).reshape(-1)
        log_rows.append({name: float(pos[k]) for k, name in enumerate(joint_names)})

df = pd.DataFrame(log_rows)
df.index.name = "step"          # 横軸＝step
df.to_csv("joint_positions.csv")

ax = df.plot()
ax.set_xlabel("step")
ax.set_ylabel("position")
plt.tight_layout()
plt.savefig("joint_positions.png", dpi=200)
print("saved: joint_positions.png, joint_positions.csv")
